...
3. **Downscale.** `cv2.resize(frame, fx=0.33, fy=0.33)` — `DETECT_SCALE`. Detection runs on the ~1/9-area image (measured 42 ms/frame against 98 ms at 0.5 on a 1080p source); box coordinates are multiplied back up only for drawing.
...
```python
owner_recently_seen = (time.time() - last_owner_seen) < owner_grace   # 1.5 s
if intruder_detected and not owner_present and not owner_recently_seen:
    unknown_streak += 1
    if unknown_streak >= streak_limit:                                # 2
        unknown_streak = 0
        if now - last_lock_time > LOCK_COOLDOWN:                      # 15 s
...
- **Locks only when all three hold:** an unrecognized face is in frame, no recognized face is in frame, and no recognized face has been in frame within the last `--owner-grace` seconds (default 1.5).
- **Owner + guest in frame → never locks.** The guest is reported in the log as "owner present with guest".
- **Owner grace (1.5 s, `--owner-grace`)** covers momentary detector dropout on the owner's face — e.g. head turn, glare, occlusion — preventing a false lock in the instant the owner is missed but a colleague behind them is not. It suppresses the streak outright, so it is also the single largest contributor to time-to-lock; raise it if false locks appear, lower it for a faster reaction. Suppressed frames now print `⏳ Unknown face ignored — owner seen Ns ago`, throttled to one line per second.
...
- **Time to lock, in practice:** measured on a 1080p camera with 14 enrolled encodings —
  ...
  | Step | Cost |
  |---|---|
  | grace-window suppression (owner seen recently) | 0–1500 ms |
  | idle `sleep(interval)` before the first qualifying frame | 0–200 ms |
  | 2 × (HOG detect 42 ms + encode 5 ms), no sleep between | 94 ms |
  | `save_snapshot` imwrite | 4 ms |
  | `lock_screen` CGSession subprocess | ~100 ms |
  | **total** | **~300 ms typical, ~1.8 s worst case** |
...
**Snapshot** (`save_snapshot()`, line 138) — writes `~/.faceguard_snapshots/intruder_YYYYMMDD_HHMMSS.jpg` from the current full-resolution frame. Because bounding boxes and `You` / `UNKNOWN (x.xx)` labels are drawn onto that same `frame` array in-place earlier in the iteration, **snapshots taken with the preview enabled contain the annotations**; with `--no-preview` they are clean camera frames. The bottom status bar is drawn after the lock block, so it never appears in a snapshot.
...
- **Third-scale detection** (`DETECT_SCALE = 0.33`) is the main CPU saving — 42 ms/frame against 98 ms at 0.5 on a 1080p source, with detection coordinates upscaled only for display. 0.25 is faster still (24 ms) but was rejected: it drops faces below ~150 px at full resolution. Verified across all 27 saved snapshots, where 0.33 matches 0.5 exactly and 0.25 misses one.
...
| `DETECT_SCALE` | `0.33` | Pre-detection downscale factor |
| `PREVIEW_WIDTH` | `320` px | Preview window width |
| `LOCK_COOLDOWN` | `15` s | Minimum gap between locks |
| `OWNER_GRACE` | `1.5` s | Suppression window after last owner sighting (`--owner-grace`) |
